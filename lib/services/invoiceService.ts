...
type CreateInvoiceBody = {
  ...
  source_type?: "INSTALLATION" | "FOLLOW_UP" | "MANUAL";
...
export async function POST(req: Request) {
  try {
    ...
    const unitPrice = toNumber(body.unit_price, 0);
    ...
    const discountRate = toNumber(body.discount_rate, 0);
    const discountAmount = subtotalAmount * (discountRate / 100);

    const taxableAmount = subtotalAmount - discountAmount;
    const taxExempt = Boolean(body.tax_exempt);
    const taxRate = taxExempt ? 0 : toNumber(body.tax_rate, 13);
    ...
    const customerSnapshotName =
      toNullableString(body.customer_snapshot_name) ||
      client.billing_name ||
    ...
    const customerSnapshotPhone =
      toNullableString(body.customer_snapshot_phone) ||
      client.billing_phone ||
      client.phone_primary ||
      null;

    const serviceSnapshotDescription =
      toNullableString(body.service_snapshot_description) ||

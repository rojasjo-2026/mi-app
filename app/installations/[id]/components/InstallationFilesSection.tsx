...
export default function InstallationFilesSection({
    ...
    async (showLoadingState = true) => {
      try {
        ...
        const res = await fetch(
          `/api/files?entity_type=installation&entity_id=${installationId}`,
          {
            cache: "no-store",
          },
        );
        ...
        console.error("Error loading installation files:", error);
        return [];
      } finally {
        if (showLoadingState) {
          setLoading(false);
  ...
  async function uploadSelectedFiles() {
    if (previewFiles.length === 0 || !installationId) return;
    ...
    try {
      ...
      for (let index = 0; index < previewFiles.length; index += 1) {
        ...
        const filePath = `installations/${installationId}/${uniqueFileName}`;
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-slate-500">
              Subí fotos, PDFs, comprobantes o documentos relacionados con esta
              instalación para conservar la trazabilidad técnica del trabajo.
            </p>
          </div>
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-6 text-sm text-slate-500">
            No hay archivos aún. Agregue evidencias para esta instalación.
          </div>
        ) : (
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">

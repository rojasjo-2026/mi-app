import { AlertTriangle, LoaderCircle, Scale } from "lucide-react";

type InventoryUnitLoadingStateProps = {
  message?: string;
};

export function InventoryUnitLoadingState({
  message = "Cargando unidades…",
}: InventoryUnitLoadingStateProps) {
  return (
...
type InventoryUnitErrorStateProps = {
  message: string;
  onRetry: () => void;
};

export function InventoryUnitErrorState({
  message,
  onRetry,
}: InventoryUnitErrorStateProps) {
  return (
...
type InventoryUnitEmptyStateProps = {
  hasFilters: boolean;
  onCreate: () => void;
  onClearFilters: () => void;
};

export function InventoryUnitEmptyState({
  hasFilters,
  onCreate,
  onClearFilters,
}: InventoryUnitEmptyStateProps) {
  return (
    <section className="rounded-lg border border-dashed border-slate-300 bg-white px-6 py-12 text-center shadow-sm">
      ...
      <h2 className="mt-4 text-base font-bold text-slate-900">
        {hasFilters ? "No encontramos unidades" : "Todavía no hay unidades"}
      </h2>

      <p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-slate-600">
        {hasFilters
          ? "Probá modificando la búsqueda o los filtros aplicados."
          : "Creá la primera unidad para registrar correctamente las cantidades del inventario."}
      </p>

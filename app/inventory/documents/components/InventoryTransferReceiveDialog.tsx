"use client";

import {
  ArrowRight,
  CheckCircle2,
  Loader2,
  PackageCheck,
  Truck,
  X,
} from "lucide-react";

import { useEffect, useState } from "react";

import { createPortal } from "react-dom";

import type { InventoryApiResponse, InventoryDocumentDetail } from "../types";

import { formatInventoryDocumentMoney } from "../utils/inventoryDocumentUi";

type InventoryTransferReceiveDialogProps = {
  open: boolean;
  detail: InventoryDocumentDetail | null;
  locale: string;
  currency: string;
  onClose: () => void;
  onReceived: (document: InventoryDocumentDetail) => void;
};

function getLocationLabel(
  location: InventoryDocumentDetail["source_location"],
  emptyLabel: string,
) {
  if (!location) {
    return emptyLabel;
  }

  return `${location.name} (${location.location_code})`;
}

export default function InventoryTransferReceiveDialog({
  open,
  detail,
  locale,
  currency,
  onClose,
  onReceived,
}: InventoryTransferReceiveDialogProps) {
  const [receiving, setReceiving] = useState(false);

  const [error, setError] = useState("");

  useEffect(() => {
    if (!open) {
      return;
    }

    setError("");
  }, [detail?.inventory_document_id, open]);

  useEffect(() => {
    if (!open) {
      return;
    }

    function handleKeyDown(event: KeyboardEvent) {
      if (event.key === "Escape" && !receiving) {
        onClose();
      }
    }

    document.addEventListener("keydown", handleKeyDown);

    return () => {
      document.removeEventListener("keydown", handleKeyDown);
    };
  }, [onClose, open, receiving]);

  if (!open || !detail) {
    return null;
  }

  const inventoryDocumentId = detail.inventory_document_id;

  const sourceLabel = getLocationLabel(
    detail.source_location,
    "Sin ubicación de origen",
  );

  const destinationLabel = getLocationLabel(
    detail.destination_location,
    "Sin ubicación de destino",
  );

  const productCount = detail.lines.length;

  const canReceive =
    detail.document_type === "TRANSFER" &&
    detail.status === "IN_TRANSIT" &&
    productCount > 0;

  async function handleReceive() {
    if (receiving || !canReceive) {
      return;
    }

    try {
      setReceiving(true);
      setError("");

      const response = await fetch(
        `/api/inventory/documents/${encodeURIComponent(
          inventoryDocumentId,
        )}/receive`,
        {
          method: "POST",
        },
      );

      const result: InventoryApiResponse<InventoryDocumentDetail> =
        await response.json();

      if (!response.ok || !result.success || !result.data) {
        throw new Error(
          result.message || "No se pudo recibir la transferencia.",
        );
      }

      onReceived(result.data);
    } catch (receiveError) {
      setError(
        receiveError instanceof Error
          ? receiveError.message
          : "Ocurrió un error al recibir la transferencia.",
      );
    } finally {
      setReceiving(false);
    }
  }

  return createPortal(
    <div className="fixed inset-0 z-[400] flex items-center justify-center p-4">
      <button
        type="button"
        aria-label="Cerrar confirmación"
        onClick={onClose}
        disabled={receiving}
        className="absolute inset-0 cursor-pointer bg-slate-950/10 disabled:cursor-not-allowed"
      />

      <section className="relative w-full max-w-xl overflow-hidden rounded-xl border border-slate-200 bg-white shadow-2xl">
        <header className="flex items-start gap-3 border-b border-slate-200 px-5 py-4">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-md bg-emerald-50 text-emerald-700">
            <PackageCheck className="h-5 w-5" aria-hidden="true" />
          </div>

          <div className="min-w-0 flex-1">
            <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-400">
              Transferencia de inventario
            </p>

            <h2 className="mt-1 text-lg font-semibold text-slate-950">
              Confirmar recepción
            </h2>

            <p className="mt-1 break-all text-xs text-slate-500">
              {detail.document_number}
            </p>
          </div>

          <button
            type="button"
            onClick={onClose}
            disabled={receiving}
            aria-label="Cerrar"
            className="inline-flex h-9 w-9 shrink-0 cursor-pointer items-center justify-center rounded-md border border-slate-200 bg-white text-slate-500 transition hover:bg-slate-50 hover:text-slate-950 disabled:cursor-not-allowed disabled:opacity-50"
          >
            <X className="h-4 w-4" aria-hidden="true" />
          </button>
        </header>

        <div className="space-y-4 p-5">
          <div className="flex items-start gap-3 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3">
            <Truck
              className="mt-0.5 h-5 w-5 shrink-0 text-emerald-700"
              aria-hidden="true"
            />

            <div>
              <p className="text-sm font-semibold text-emerald-900">
                Los productos ingresarán a la ubicación de destino.
              </p>

              <p className="mt-1 text-xs leading-5 text-emerald-800">
                Se generará un movimiento de entrada por cada producto y la
                transferencia quedará recibida.
              </p>
            </div>
          </div>

          {error ? (
            <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
              {error}
            </div>
          ) : null}

          <dl className="divide-y divide-slate-100 rounded-lg border border-slate-200 px-4">
            <div className="flex items-center justify-between gap-4 py-3">
              <dt className="text-sm text-slate-500">Productos</dt>

              <dd className="text-sm font-semibold text-slate-950">
                {productCount}
              </dd>
            </div>

            <div className="flex items-center justify-between gap-4 py-3">
              <dt className="text-sm text-slate-500">
                Valor de la transferencia
              </dt>

              <dd className="text-sm font-semibold tabular-nums text-slate-950">
                {formatInventoryDocumentMoney(
                  detail.total_cost,
                  locale,
                  currency,
                )}
              </dd>
            </div>

            <div className="flex items-center justify-between gap-4 py-3">
              <dt className="text-sm text-slate-500">Estado resultante</dt>

              <dd className="inline-flex items-center gap-1.5 text-sm font-semibold text-emerald-700">
                <CheckCircle2 className="h-4 w-4" aria-hidden="true" />
                Recibido
              </dd>
            </div>
          </dl>

          <div className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3">
            <p className="text-xs font-semibold uppercase tracking-[0.08em] text-slate-400">
              Recorrido
            </p>

            <div className="mt-2 flex items-center gap-2 text-sm font-semibold text-slate-700">
              <span className="min-w-0 truncate">{sourceLabel}</span>

              <ArrowRight
                className="h-4 w-4 shrink-0 text-slate-400"
                aria-hidden="true"
              />

              <span className="min-w-0 truncate">{destinationLabel}</span>
            </div>
          </div>

          {!canReceive ? (
            <p className="text-sm font-medium text-red-600">
              Solo pueden recibirse transferencias en tránsito que contengan
              productos.
            </p>
          ) : null}
        </div>

        <footer className="flex items-center justify-end gap-2 border-t border-slate-200 bg-slate-50 px-5 py-3">
          <button
            type="button"
            onClick={onClose}
            disabled={receiving}
            className="inline-flex h-9 cursor-pointer items-center justify-center rounded-md border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50"
          >
            Cancelar
          </button>

          <button
            type="button"
            onClick={() => void handleReceive()}
            disabled={receiving || !canReceive}
            className="inline-flex h-9 cursor-pointer items-center justify-center gap-2 rounded-md bg-emerald-600 px-4 text-sm font-semibold text-white transition hover:bg-emerald-700 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-emerald-100 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {receiving ? (
              <Loader2 className="h-4 w-4 animate-spin" aria-hidden="true" />
            ) : (
              <PackageCheck className="h-4 w-4" aria-hidden="true" />
            )}

            {receiving ? "Recibiendo..." : "Recibir transferencia"}
          </button>
        </footer>
      </section>
    </div>,
    document.body,
  );
}

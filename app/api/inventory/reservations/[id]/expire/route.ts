...
import { expireInventoryReservation } from "@/lib/inventory/reservations/inventoryReservationExpiration.service";

import type { InventoryServiceResult } from "@/lib/inventory/shared/inventoryServiceResult.types";

type ExpirationRouteContext = {
  params: Promise<{
    id: string;
  }>;
};
...
export async function POST(
  ...
  try {
    ...
    const result = await expireInventoryReservation(id, parsedBody.value);

    return respond(result);
  } catch (error) {
    console.error("POST /api/inventory/reservations/[id]/expire error:", error);

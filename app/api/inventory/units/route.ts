...
import {
  createUnitOfMeasure,
  getUnitOfMeasuresFromSearchParams,
...
export async function GET(request: Request) {
  ...
  const result = await getUnitOfMeasuresFromSearchParams(searchParams);
...
export async function POST(request: Request) {
  try {
    ...
    const result = await createUnitOfMeasure(body);
    ...
    console.error("POST /api/inventory/units error:", error);
